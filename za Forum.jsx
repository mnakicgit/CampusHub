    ...
    return (
        <div className="forum-container">
            ...
            <div className={`forum-sidebar ${isSidebarOpen ? 'open' : 'closed'}`} >
                <button className="forum-button" onClick={() => { setShowCreateForum(true); setShowAllForums(false); setShowFavorites(false); setShowCreatedByMe(false); }}>
                    Start a Discussion
                </button>
            ...
            <div className="forum-main">
                ...
                {showWelcomeMessage && (
                    <div className="welcome-message">
                        <h3>Pozdrav svima! Dobrodošli na forum! Ovdje imate priliku kreirati vlastite teme, postavljati pitanja, 
							ali i komentirati forume koje su kreirali vaši kolege studenti. Ovaj forum je mjesto na kojem možete r
						azmjenjivati mišljenja, diskutirati o temama vezanim uz fakultet, predmete, kolokvije i sve što vas zanima. 
						Iskoristite ga kao platformu za povezivanje s drugima, bilo da su već studenti ili tek planiraju postati. 
						Želimo vam puno uspjeha i zanimljivih rasprava!</h3>
                        ...
                        <div className="file-upload-container">
                            ...
                            <div className="file-info">
                                {uploadedFile ? (
                                    ...
                                    <p className="no-file-chosen">No file chosen</p>
                                )}
                            </div>
                        </div>
...
{showFavorites && (
    <div>
        <h2>Favorites</h2>
        {favorites.length === 0 ? <p>No favorite forums yet.</p> : (
            forums.filter(forum => favorites.includes(forum.id)).map((forum) => (
            ...
            {favorites.includes(forum.id) ? "Remove from favorites" : "Add to favorites"}
                ...
                {showCreatedByMe && (
                    <div>
                        ...
                        {myForums.length === 0 ? <p>No forums created yet.</p> : (
                            myForums.map((forum) => (
                                <div key={forum.id} className="forum-item" onClick={() => openForum(forum)}>
                                    <h3>{forum.title}</h3>
                                    <p>{forum.comments.length} comments</p>
                ...
                {selectedForum && (
                    <div className="forum-details">
                        ...
                        <p>Created by: <b>{selectedForum.author}</b>, Created at: <i>{selectedForum.createdAt}</i></p>
                                    ...
                                    <div key={index} className="comment">
                                        ...
                                        {comment.file && (
                                            <div className="file-preview">
                                                <p>File: <a href={URL.createObjectURL(comment.file)} download>{comment.file.name}</a></p>
                                            </div>
                                        )}
                                    </div>
                                ))
                            ...
                            {loggedInUser && (
                                <>
                                    <textarea
                                        ...
                                        value={newComment}
                                        onChange={(e) => setNewComment(e.target.value)}
                                        onKeyDown={(e) => handleCommentKeyPress(e, selectedForum.id)}
                                    />
									 <div className="file-upload-container">
                                <label htmlFor="file-upload" className="file-upload-label">
                                    <span>📎</span> Attach a file
